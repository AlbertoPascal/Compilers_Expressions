...
def drawGraph():
    ...
    print("My edge list for the NFA is now " + str(edge_labels))
    ...
    if(NFAisDFA == False):
        ...
        print("My edge list for the DFA is now: ")
        print(edge_labels2)
        #print("Edge_label2: ")
...
def cleanEclosure(node):
    nodeList = list(filter(lambda a: a != ',', node))
    nodeList.sort()
    nodeList = list(dict.fromkeys(nodeList))
    node = ""
    for nodeL in nodeList:
        node += nodeL + ","
    node = node[:-1]
    return node
...
def getEclose(currentRead):
    ...
    newNodeArr = []
    DFA.append(dict())
    DFA[-1]["Node"] = currentRead
    added_nodes = []
    pending_nodes = []
    currentRead = currentRead.split(",")
    newNode = ""
    #print(nodeForE)
    not_added_in_a_while = 0
    prev_length = 0
    for key in edgesDFA:
        for node in currentRead:
            currNode = transitionTable[int(node)][key]
            if (currNode != ""):    
                if currNode[-1] == ",":
                    currNode = currNode[:-1]
                    newNode += currNode
                    pending_nodes.append(currNode)
                    added_nodes.append(currNode)
                    while(len(pending_nodes) > 0):
                        currNode = transitionTable[int(pending_nodes.pop())]["E"]
                        if not str(currNode) in newNode:
                            newNode += ","+currNode
                            temp_nodes = currNode.split(',')
                            
                            #print("would need to add " + str(temp_nodes))
                            for elem in temp_nodes:
                                if elem not in added_nodes:
                                    added_nodes.append(elem)
                                    pending_nodes.append(elem)
                            if len(added_nodes) == prev_length:
                                not_added_in_a_while +=1
                            if not_added_in_a_while > 5:
                                break

        
        newNodeArr.append(cleanEclosure(newNode))
        DFA[-1][key] = newNodeArr[-1]
        newNode = ""
        added_nodes = []
        pending_nodes = []                
    #print("---DFAclean")
    ...
    return newNodeArr
    #checkDestinations(newNodeArr)          
                    #newNode += transitionTable[int(currentNode)]["E"]
...
def NFAtoDFA():
    ...
    if (transitionTable[first_node_arr[0][0]]["E"] != ""):
        nodeArr = [str(first_node_arr[0][0]) + "," +transitionTable[first_node_arr[0][0]]["E"]]
        node_arr = getEclose(nodeArr[0])
        checkDestinations(node_arr)
...
def validateString(string):
	string = list(string)
	currentNode = DFA[0]["Node"]
	for letter in string:
		if letter in edgesDFA:
			for node in DFA:
				if node["Node"] == currentNode:
					nextNode = DFA.index(node)
					currentNode = DFA[nextNode][letter]
					#print("My current letter is " + str(letter) + " in node " + str(currentNode))
					break
			if(currentNode not in DFA_final_nodes):
				valid = False
			else:
				valid = True
			'''if letter in DFA[currentNode]:
				current_node = DFA[current_node][letter]
				print(currentNode)'''
		else:
			if letter == ' ' and currentNode in DFA_final_nodes:
				valid = True
			else:
				valid = False
				break
...
looped_or = False
Expression_count=0
regex = str(input("Insert a Regular Expression to build: "))
...
cleanDFA()
getFinalNodes()
print("My final nodes are")
print(DFA_final_nodes)
drawGraph()
print(DFA_final_nodes)
string_to_interpret = str(input("Insert a string to verify on DFA: "))
validateString(string_to_interpret)
