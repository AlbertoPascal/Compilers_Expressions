...
def handleAsteriskPlus(invoker_char):
    global indiv_connector_nodes, Node_count, Connection_list, Expression_count, last_node_arr, first_node_arr, pending_connections
    ###print("fonud")
...
def interpretSingleInstruct(string_to_interpret):
    global indiv_connector_nodes, interpretation_called, Node_count, Connection_list, Expression_count, last_node_arr, first_node_arr, pending_connections, handled_pipe
    ...
    for i in string_to_interpret:
        ...
        if (i == '*' or i == '+'):
            ...
            if(first_time_flag):
                indiv_connector_nodes[string_to_interpret] = []
                indiv_connector_nodes[string_to_interpret] = (Node_count, "NA")
                first_time_flag=False
            if(i==string_to_interpret[-1]):
                #Im on my last char sent. Need to store my ending connection:
                indiv_connector_nodes[string_to_interpret] = (indiv_connector_nodes[string_to_interpret][0],Node_count +1)
            ##print("Added" + str((Node_count, i, Node_count + 1)))
            Node_count = Node_count + 1
            print("My indiv_connectors are: " + str(indiv_connector_nodes))
...
#        pending_connections=False
    Connection_list.append("EoL")
    Node_count+=1 #Next time I call myself I'm a new individual node fraction. 
    ##print("My start node is now " + str(first_node_arr))
    ##print("My end node is now " + str(last_node_arr))
def preparePipe():
